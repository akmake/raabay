    ...
    <div dir="rtl" style={{ background: V.white, color: V.ink, fontFamily: V.sans, WebkitFontSmoothing: 'antialiased' }}>
      ...
      <main id="main-content" tabIndex={-1} style={{ paddingTop: 60 }}>
        ...
        <section style={{ background: V.white, padding: m ? '52px 0 44px' : '88px 0 80px' }}>
          <div style={{ ...wrap, display: 'grid', gridTemplateColumns: m ? '1fr' : '1fr 420px', gap: m ? 36 : 80, alignItems: 'center' }}>
            <div>
              ...
              <h1 style={{ fontFamily: V.serif, fontWeight: 600, fontSize: m ? 30 : 50, lineHeight: 1.25, color: V.ink, marginBottom: 24 }}>
            ...
            <div style={{ order: m ? -1 : 0 }}>
              <div style={{ position: 'relative', borderRadius: 4, overflow: 'hidden', boxShadow: '0 24px 60px -16px rgba(20,34,63,.35)' }}>
                <img src="/הרבי.jpg" alt="הרבי מליובאוויטש" style={{ display: 'block', width: '100%', height: m ? 260 : 460, objectFit: 'cover', objectPosition: 'top' }} />
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'linear-gradient(transparent, rgba(20,34,63,.65))', padding: '32px 20px 18px', fontFamily: V.serif, fontSize: 15, color: '#fff', textAlign: 'center' }}>
        ...
        <section style={{ background: V.navy, padding: m ? '52px 0' : '72px 0' }}>
          <div style={{ ...wrap, textAlign: 'center', position: 'relative' }}>
            <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 70% 120% at 50% 0%, rgba(176,141,74,.18), transparent 65%)', pointerEvents: 'none' }} />
            <div style={{ position: 'relative', zIndex: 1 }}>
              <div style={{ fontFamily: V.serif, fontSize: m ? 28 : 46, fontWeight: 700, color: '#fff', lineHeight: 1.3, marginBottom: 18 }}>״לִבְנֵי אֲנִי צָרִיךְ״</div>
              <p style={{ fontSize: m ? 15 : 17, color: V.mist, maxWidth: '34em', margin: '0 auto', lineHeight: 1.85 }}>
                בשעת פטירתו נשאל הרבי הקודם מה ברצונו — וענה: "לבני אני צריך". הרבי ממשיך לחפש את הקשר עם כל אחד ואחת. הכתיבה אליו היא המענה לאותה קריאה.
              </p>
            </div>
          </div>
        </section>
          ...
          <div style={wrap}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: m ? '1fr' : 'repeat(3,1fr)', gap: m ? 28 : 40 }}>
              {[
                ...
                { num: '03', title: 'המכתב מגיע', body: 'כל מכתב מודפס ונשלח לאוהל הקדוש שבקווינס, ניו יורק — ומונח על ציון הרבי. כיחידות ממש.' },
              ].map((item) => (
