...
const defaults = {
  fontSize: 0,        // -1=קטן, 0=רגיל, 1=גדול, 2=גדול מאוד
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
      ...
      <button
        ...
        style={{
          ...
          background: value ? '#1e1a17' : '#d4c8b8',
    ...
    <>
      ...
      {open && (
        <div
          ...
          dir="rtl"
          style={{
            position: 'fixed', bottom: 88, left: 24, zIndex: 9001,
        ...
        >
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }} role="group" aria-label="גודל טקסט">
              <button
                ...
                style={settings.fontSize === -1 ? BTN_ACTIVE : BTN}
                aria-pressed={settings.fontSize === -1}
              >א–</button>
              <button
                onClick={() => update({ fontSize: 0 })}
                style={settings.fontSize === 0 ? BTN_ACTIVE : BTN}
            ...
            <div style={{ display: 'flex', gap: 6 }} role="group" aria-label="ניגוד צבעים">
              {[['normal', 'רגיל'], ['high', 'ניגוד גבוה']].map(([val, label]) => (
                <button
                  ...
                  style={settings.contrast === val ? BTN_ACTIVE : BTN}
          ...
          <Toggle
            id="a11y-no-anim"
            label="עצור אנימציות"
            value={settings.noAnimations}
            onChange={() => update({ noAnimations: !settings.noAnimations })}
          />
          <Toggle
            id="a11y-links"
            label="הדגש קישורים"
            value={settings.underlineLinks}
            onChange={() => update({ underlineLinks: !settings.underlineLinks })}
          />
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 7, border: '1px solid #d4c8b8',
              background: 'none', color: '#5c5550',
